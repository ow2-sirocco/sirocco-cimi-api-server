...
/**
 * Manage ACTION request of Network.
 */
@org.ow2.sirocco.cimi.server.manager.Manager("CimiManagerActionNetwork")
public class CimiManagerActionNetwork extends CimiManagerAbstract {
    ...
    protected Object callService(final CimiContext context, final Object dataService) throws Exception {
        ...
        switch (type) {
        case START:
            out = this.manager.startNetwork(context.getRequest().getId(), cimiAction.getProperties());
            break;
        case STOP:
            out = this.manager.stopNetwork(context.getRequest().getId(), cimiAction.getProperties());
            break;
        default:
