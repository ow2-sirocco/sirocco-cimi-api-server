...
/**
 * Manage READ request of ForwardingGroups collection of a System.
 */
@org.ow2.sirocco.cimi.server.manager.Manager("CimiManagerReadSystemForwardingGroupCollection")
public class CimiManagerReadSystemForwardingGroupCollection extends CimiManagerReadAbstract {
    ...
    protected Object callService(final CimiContext context, final Object dataService) throws Exception {
        ...
        if (false == context.hasParamsForReadingCollection()) {
            out = this.manager.getEntityListFromSystem(context.getRequest().getIdParent(), SystemForwardingGroup.class);
        } else {
            QueryResult<?> result = this.manager.getEntityListFromSystem(context.getRequest().getIdParent(),
                SystemForwardingGroup.class, context.valueOfFirst(), context.valueOfLast(), context.valuesOfFilter(),
    ...
    protected void convertToResponse(final CimiContext context, final Object dataService) throws Exception {
        CimiSystemForwardingGroupCollectionRoot cimi = (CimiSystemForwardingGroupCollectionRoot) context.convertToCimi(
            dataService, CimiSystemForwardingGroupCollectionRoot.class);
