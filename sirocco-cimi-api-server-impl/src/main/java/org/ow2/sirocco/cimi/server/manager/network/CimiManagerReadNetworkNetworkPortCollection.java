...
/**
 * Manage READ request of NetworkPorts collection of a Network.
 */
@org.ow2.sirocco.cimi.server.manager.Manager("CimiManagerReadNetworkNetworkPortCollection")
public class CimiManagerReadNetworkNetworkPortCollection extends CimiManagerReadAbstract {
    ...
    protected Object callService(final CimiContext context, final Object dataService) throws Exception {
        ...
        if (false == context.hasParamsForReadingCollection()) {
            out = this.manager.getNetworkNetworkPorts(context.getRequest().getIdParent());
        } else {
            QueryResult<?> results = this.manager.getNetworkNetworkPorts(context.getRequest().getIdParent(),
    ...
    protected void convertToResponse(final CimiContext context, final Object dataService) throws Exception {
        CimiNetworkNetworkPortCollectionRoot cimi = (CimiNetworkNetworkPortCollectionRoot) context.convertToCimi(dataService,
            CimiNetworkNetworkPortCollectionRoot.class);
