...
/**
 * Manage READ request of Systems collection of a System.
 */
@org.ow2.sirocco.cimi.server.manager.Manager("CimiManagerReadSystemSystemCollection")
public class CimiManagerReadSystemSystemCollection extends CimiManagerReadAbstract {
    ...
    protected Object callService(final CimiContext context, final Object dataService) throws Exception {
        ...
        if (false == context.hasParamsForReadingCollection()) {
            out = this.manager.getEntityListFromSystem(context.getRequest().getIdParent(), SystemSystem.class);
        } else {
            QueryResult<?> result = this.manager.getEntityListFromSystem(context.getRequest().getIdParent(),
                SystemSystem.class, context.valueOfFirst(), context.valueOfLast(), context.valuesOfFilter(),
    ...
    protected void convertToResponse(final CimiContext context, final Object dataService) throws Exception {
        CimiSystemSystemCollectionRoot cimi = (CimiSystemSystemCollectionRoot) context.convertToCimi(dataService,
            CimiSystemSystemCollectionRoot.class);
