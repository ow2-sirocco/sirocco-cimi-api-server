...
/**
 * Manage READ request of NetworkPort Configuration.
 */
@org.ow2.sirocco.cimi.server.manager.Manager("CimiManagerReadNetworkPortConfiguration")
public class CimiManagerReadNetworkPortConfiguration extends CimiManagerReadAbstract {
    ...
    protected Object callService(final CimiContext context, final Object dataService) throws Exception {
        NetworkPortConfiguration out = null;
        if (false == context.hasParamSelect()) {
            out = this.manager.getNetworkPortConfigurationByUuid(context.getRequest().getId());
        } else {
            out = this.manager.getNetworkPortConfigurationAttributes(context.getRequest().getId(), context.valuesOfSelect());
        }
        return out;
    }
