...
/**
 * Manage READ request of Events collection of an EventLog.
 */
@org.ow2.sirocco.cimi.server.manager.Manager("CimiManagerReadEventLogEventCollection")
public class CimiManagerReadEventLogEventCollection extends CimiManagerReadAbstract {
    ...
    protected Object callService(final CimiContext context, final Object dataService) throws Exception {
        ...
        if (false == context.hasParamsForReadingCollection()) {
            out = this.manager.getEvents(context.getRequest().getIdParent());
        } else {
            QueryResult<?> results = this.manager.getEvents(context.getRequest().getIdParent(), context.valueOfFirst(),
    ...
    protected void convertToResponse(final CimiContext context, final Object dataService) throws Exception {
        CimiEventCollectionRoot cimi = (CimiEventCollectionRoot) context.convertToCimi(dataService,
            CimiEventCollectionRoot.class);
